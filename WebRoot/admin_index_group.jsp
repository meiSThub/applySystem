...
<html>
    ...
    <div class="container">
      <div class="tabbable tabs-left">
        <ul id="myTab" class="nav nav-tabs">
          ...
          <li><a href="#event-approve?pageChildren=enventApprove" data-toggle="tab">活动审批</a></li>
          <li class="active"><a href="admin_group.html" data-toggle="tab">查看单位用户</a></li>
          <li><a href="adminEventDetail?pageChildren=personUser" data-toggle="tab">查看个人用户</a></li>
        </ul>
  ...
  <!--查看单位用户 -->
  <% 
	String pageChildren=request.getParameter("pageChildren");
	if("groupUser".equals(pageChildren)){
	
  %>
          <div class="tab-pane fade1 in active" id="group-user">
            查询单位：
            <select class="span4">
              <option>教务处</option>
              <option>图书馆</option>
            ...
            <div class="row-fluid">
              ...
              <% 
				...
				//Team teamList=new ArrayList();		
				if(requestPage==null)    requestPage="1";
				//总页数
				pageCount=teamDaoImp.getPageNum();
				currentPage=Integer.parseInt(requestPage);
				begin=(currentPage-1)*size;
				try{
			  		for(Team team:teamDaoImp.queryAllTeam(1)){
              %>
              <div class="span11">
                <h4><a href="adminEventDetail?teamId=<%=team.getTeamId() %>&pageChildren=groupUser"><%=team.getTeamName() %></a>
                  <small>
                    <a href="adminEventDetail?teamId=<%=team.getTeamId() %>&pageChildren=groupUser" class="button btn btn-primary pull right">进入</a>
                  </small>
                </h4>
              </div>
              <% 
